...
DROP TABLE telemedicineprovider1 PURGE;
DROP TABLE beneficiarydata1 PURGE;

-- Create table for BeneficiaryData1 data
CREATE TABLE beneficiarydata1 (
  race VARCHAR(255),
...
-- Create table for TelemedicineProvider1 data
CREATE TABLE telemedicineprovider1 (
  round INT,
  ...
  FOREIGN KEY (race, urbanization) REFERENCES beneficiarydata1(race, urbanization));

-- SQL Script for Bulk Insert into Oracle DB

INSERT ALL
    INTO beneficiarydata1 (race, urbanization, enrollment_status, total_bene_th_elig, total_partb_enrl, total_bene_telehealth, pct_telehealth)
...
SELECT 1 FROM DUAL;
INSERT ALL
    INTO telemedicineprovider1 (round, indicator, race, urbanization, education, samplesize, sizevalue,
...
-- 1. Average telehealth usage by race
SELECT 
  tp.race,
...
-- 4. Combine with beneficiary data (racial & geographic interaction)
SELECT 
  tp.urbanization,
  AVG(tp.percent_yes) AS avg_telehealth_usage,
